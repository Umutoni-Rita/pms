...
# Configure the serial port (adjust '/dev/ttyACM0' to your Arduino's port)
    ...
    while True:
        if ser.in_waiting > 0:
            ...
            if line.startswith("DATA:"):
                ...
                # Check if cash is more than 500
                if cash <= 500:
                    print_boxed_message("Error: Insufficient Balance", "!")
                    print(f"[{get_timestamp()}] Balance ({cash} units) must be > 500 units.\n")
                    continue
